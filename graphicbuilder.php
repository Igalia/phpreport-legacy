...
if ($validflag) {
  ...
  if (($first_date != "") && validate_date_web(date_sql_to_web($first_date))) {
    $exists_graph_data=true;
    $first_date=date_sql_to_web($first_date);
  }
}
if ($exists_graph_data) {
  ...
  $start_date = date_arrayDMA_to_web(array(1, $first_date_arrayDMA[1], $first_date_arrayDMA[2]));

  /* end date: the first day of the next month starting at current day */
  $today=getdate(time());
  $this_month="01/".$today["mon"]."/".$today["year"];
  $end_date=day_month_moved($this_month, 1);

  /* loop retrieving monthly data, since start_date to end_date */
  $monthly_hours = array();
  for ($current_date = $start_date; $current_date != $end_date; $current_date=day_month_moved($current_date, 1)) {
    ...
    if ($g_flag == "PROJECT") {
      $data=@pg_exec($cnx, $query="SELECT uid AS data_id, SUM( _end - init ) / 60.0 AS add_hours FROM task WHERE ( ( _date >= '"
		     .$current_date."' AND _date < '".$current_date."'::date + '1 month'::interval ) AND name = '"
		     .$g_id."' ) GROUP BY uid ORDER BY uid ASC")  or die($die);
    } else if ($g_flag = "PERSON") {
      $data=@pg_exec($cnx, $query="SELECT CASE name WHEN '' THEN '"._("unknown project")."' ELSE name END AS data_id, SUM( _end - init ) / 60.0 AS add_hours FROM task WHERE ( ( _date >= '"
		     .$current_date."' AND _date < '".$current_date."'::date + '1 month'::interval ) AND uid = '"
  ...
  if ($g_flag == "PROJECT") {
    ...
    $projects=@pg_exec($cnx,$query="SELECT DISTINCT uid FROM task WHERE name = '".$g_id."' ORDER BY uid ASC");
    ...
    $users=@pg_exec($cnx,$query="SELECT DISTINCT CASE name WHEN '' THEN '"._("unknown project")."' ELSE name END FROM task WHERE uid = '".$g_id."' ORDER BY name ASC")
      or die($die);
